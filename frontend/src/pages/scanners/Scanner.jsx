...
const Scanner = ({ statergy }) => {
    ...
    useEffect(() => {
        const emarsi = "/intraday-emarsi-signals";
        const pivot = "/intraday-pivot-signals";

        let endpoint

        if (statergy === "EMARSI") {
            endpoint = emarsi;
        } else if (statergy === "IntradayPivot") {
            endpoint = pivot;
        } else {
            console.error("Unknown strategy:", statergy);
            return;
        }
                        ...
                        {selectedSymbolData && selectedSymbolData.length > 0 ? (
                            selectedSymbolData.map((signal, index) => (
                                <div key={index} className='p-2 border border-sky-800 rounded-lg bg-neutral-800'>
                                    ...
                                    <div>Entry Price: {numeral(signal.entry_price).format("0,0.00")}</div>
                                    <div>Target 1: {numeral(signal.target1).format("0,0.00")}</div>
                                    <div>Target 2: {numeral(signal.target2).format("0,0.00")}</div>
                                    <div>Target 3: {numeral(signal.target3).format("0,0.00")}</div>
                                    <div>Stop Loss: {numeral(signal.stop_loss).format("0,0.00")}</div>
                                    <div>ATR: {numeral(signal.atr).format("0,0.00")}</div>
                                </div>
                            ))
                        ) : (
