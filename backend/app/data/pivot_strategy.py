...
def main():
    ...
    files = [f for f in os.listdir("nse/15m") if f.endswith('.csv')]
    if not files:
        print("No CSV files found in nse/15m directory.")
        return
...
def pivot_strategy(file) -> pd.DataFrame:
    # === Load your 15-minute data ===
    df = pd.read_csv(f"nse/15m/{file}", parse_dates=["date"])
    ...
    df["UpTrend"] = (df["close"] > (df["low"].rolling(20).min() + 2 * df["ATR"])) & (df["EMA20"] > df["EMA40"])
    df["DnTrend"] = (df["close"] < (df["high"].rolling(20).max() - 2 * df["ATR"])) & (df["EMA20"] < df["EMA40"])
    ...
    # === Export ===
    signals = signals[signals["date"] == signals["date"].max()]
    return signals

if __name__ == "__main__":
    main()
    print("Pivot strategy completed.")
